...
| `--remote` | **(Required)** | Target URL (e.g., `http://localhost:8787` or `https://your-worker.workers.dev`). |
...
```bash
python health_check_action.py --link https://example.com --remote https://your-worker.workers.dev
```
