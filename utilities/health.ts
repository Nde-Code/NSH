...
import type { Env, RuntimeConfig, HealthCheckResult } from '../types/types.ts';
...
export async function handleHealthCheck(env: Env, config: RuntimeConfig): Promise<Response> {
    ...
    if (!configValid)
        return createJsonResponse({ status: 'unhealthy', timestamp, checks, message: MSG.WRONG_CONFIG.error }, 503);

    const secretDbBase: string = config.FIREBASE_URL + config.FIREBASE_HIDDEN_PATH;

    const { data: counterValue, error: fbError } = await readInFirebaseRTDB<number>(
        secretDbBase,
    ...
    const hasAvailableFirebaseCapacity: boolean =
        typeof counterValue === 'number' && counterValue < config.FIREBASE_ENTRIES_LIMIT;

    const systemDown: boolean = !checks.config_valid || !checks.firebase_reachable;

    const degraded: boolean =
        !systemDown && (!checks.kv_store_available || !checks.counter_accessible || !hasAvailableFirebaseCapacity);

    const allHealthy: boolean = !systemDown && !degraded;

    const status: 'healthy' | 'degraded' | 'unhealthy' =
        allHealthy === true ? 'healthy' : degraded === true ? 'degraded' : 'unhealthy';
    ...
    const result: HealthCheckResult = {
        ...
        message:
            allHealthy === true
                ? MSG.SYSTEM_OK.success
                : degraded === true
                  ? MSG.SYSTEM_DEGRADATION.warning
                  : MSG.SYSTEM_DOWN.error
    };
