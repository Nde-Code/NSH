...
export async function handleHealthCheck(env: Env, config: RuntimeConfig): Promise<Response> {
    ...
    const hasAvailableFirebaseCapacity: boolean = typeof counterValue === "number" && counterValue < config.FIREBASE_ENTRIES_LIMIT; 

    const systemDown: boolean = !checks.config_valid || !checks.firebase_reachable;
    
    const degraded: boolean = !systemDown && (!checks.kv_store_available || !checks.counter_accessible || !hasAvailableFirebaseCapacity);
