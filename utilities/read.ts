...
export async function readInFirebaseRTDB<T>(
    ...
    try {
        ...
        if (!res.ok) {
            printLogLine('ERROR', `Read operation failed: Firebase request returned status ${res.status}.`);
        ...
        printLogLine('ERROR', `Read operation failed: Firebase request failed for ${pathTo || 'root'}.`);
