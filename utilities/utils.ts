import type { StaticConfig, RuntimeConfig, NumericConfigKeys } from '../types/types.ts';
...
function getIdRegex(shortUrlIdLength: number): RegExp {
    return new RegExp(`^[a-zA-Z0-9_-]{${shortUrlIdLength}}$`);
}
...
export function isConfigValidWithMinValues(
    config: StaticConfig,
    rules: Partial<Record<NumericConfigKeys, number>>
): boolean {
...
export async function applyRateLimit(req: Request, activeConfig: RuntimeConfig): Promise<Response | null> {
    ...
    const hashedIP: string = await hashIP(ip, activeConfig.IP_HASH_SALT);

    if (!(await checkTimeRateLimit(hashedIP, activeConfig.RATE_LIMIT_INTERVAL_S)))
        return createJsonResponse(MSG.RATE_LIMIT_EXCEEDED(activeConfig.RATE_LIMIT_INTERVAL_S), 429);
